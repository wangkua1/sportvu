...
from utils import (pictorialize_team, pictorialize_fast, 
                make_3teams_11players, make_reference, scale_last_dim,wrapper_concatenated_last_dim)
...
class EncDecExtractor(BaseExtractor):
    ...
    def _format(self, x,  data_format):
        ...
        if data_format == 'location':
            return wrapper_concatenated_last_dim(scale_last_dim, x[:, 1:])
        if data_format == 'acceleration':
